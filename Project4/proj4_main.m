...
[BERnfo1(nn,jj,ii), BERnfo2(nn,jj,ii)]= JSL_MIMO_OFDM_NF(rate(jj),numBytes,snrdb,fD,CM(nn));
[BERpfo1(nn,jj,ii),BERpfo2(nn,jj,ii)] = JSL_MIMO_OFDM_PC(rate(jj),numBytes,snrdb,fD,CM(nn));
[BERzfo1(nn,jj,ii),BERzfo2(nn,jj,ii)] = JSL_MIMO_OFDM_ZF(rate(jj),numBytes,snrdb,fD,CM(nn));
[BERmo1(nn,jj,ii),BERmo2(nn,jj,ii)] = JSL_MIMO_OFDM_MMSE(rate(jj),numBytes,snrdb,fD,CM(nn));
        end
    end
end
close(wb)
%
BERnfo1 = mean(BERnfo1,3)
BERpfo1 = mean(BERpfo1,3)
BERzfo1 = mean(BERzfo1,3)
BERmo1 = mean(BERmo1,3)

BERnfo2 = mean(BERnfo2,3)
BERpfo2 = mean(BERpfo2,3)
BERzfo2 = mean(BERzfo2,3)
BERmo2 = mean(BERmo2,3)
%%
N = 1e4;
numIter = 100;
A = NaN(numIter,1);

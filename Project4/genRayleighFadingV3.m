...
function r = genRayleighFadingV3(N,fD,numChan)
% N = 11;
% fD = 1;
% numChan = 4;
...
% improved speed by getting rid of for loop, removed plotting

ssbfdn = randn(floor(N/2),2*numChan)+1j*randn(floor(N/2),...
    2*numChan); % single sideband frequency domain noise
% checking length of data
if isequal(mod(N,2),0)
    fdn = [conj(flipud(ssbfdn));ssbfdn];
elseif isequal(mod(N,2),1)
    fdn = [conj(flipud(ssbfdn));zeros(1,2*numChan);ssbfdn];
end
%
df = (2*fD)/(N-1);
